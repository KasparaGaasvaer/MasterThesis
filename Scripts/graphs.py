    ...
    def make_skn_graphs(self):
        ...
        for slice in self.slices.keys():
            if slice == "1":
                ...
                print(f"Time spent appending edges is {time_edges_end-time_edges_start:0.4f} s")
                ...
                print(f"Time spent converting edges is {T_conv_edge_2_adj_end-T_conv_edge_2_adj_start:0.4f} s")

                T_CC_start = time.perf_counter()
                labels = get_connected_components(G)
                T_CC_end = time.perf_counter()
                print(f"Time spent getting connected comp is {T_CC_end-T_CC_start:0.4f} s")
                ...
                print(f"Time spent saving img is {T_save_img_end-T_save_img_start:0.4f} s")

                cairosvg.svg2pdf(url= filename_skn + ".svg", write_to=filename_skn + ".pdf")
